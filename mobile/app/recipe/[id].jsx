...
const RecipeDetailScreen = () => {
  ...
  useEffect(() => {
    const checkIfSaved = async () => {
      try {
        ...
        const isRecipeSaved = favorites.some((fav) => fav.recipeId === Number.parseInt(recipeId))
  ...
  const handleReadInstructions = async () => {
    ...
    try {
      // Read recipe title first
      await SpeechService.speak(`Recipe: ${recipe.title}. Let's begin cooking.`, {
        rate: 0.9,
      })

      // Read each instruction step by step
      for (let i = 0; i < recipe.instructions.length; i++) {
        setCurrentStep(i)
        const stepText = `Step ${i + 1}. ${recipe.instructions[i]}`
      ...
      await SpeechService.speak("Recipe instructions complete. Enjoy your meal!", {
        rate: 0.9,
      })
    } catch (error) {
      console.error("Error reading instructions:", error)
      Alert.alert("Error", "Failed to read instructions. Please try again.")
    } finally {
      setIsSpeaking(false)
      setCurrentStep(0)
    }
  ...
  const handleReadStep = async (stepIndex) => {
    const stepText = `Step ${stepIndex + 1}. ${recipe.instructions[stepIndex]}`

    await SpeechService.speak(stepText, {
      rate: 0.85,
    })
  }
    ...
    <View style={recipeDetailStyles.container}>
      <ScrollView>
        ...
        <View style={recipeDetailStyles.contentSection}>
              ...
              {recipe.instructions.map((instruction, index) => (
                <View
                  ...
                  style={[
                    ...
                    isSpeaking && currentStep === index && recipeDetailStyles.activeInstructionCard,
                  ]}
                >
                  <LinearGradient
          ...
          <TouchableOpacity style={recipeDetailStyles.voiceButton} onPress={handleReadInstructions}>
            <LinearGradient colors={["#9C27B0", "#673AB7"]} style={recipeDetailStyles.buttonGradient}>
              <Ionicons name={isSpeaking ? "stop-circle" : "volume-high"} size={20} color={COLORS.white} />
              <Text style={recipeDetailStyles.buttonText}>
                {isSpeaking ? "Stop Reading" : "Read Instructions Aloud"}
              </Text>
            </LinearGradient>
          </TouchableOpacity>
